function plotfft(signal, yLabel, titleText)
    ...
    idx_05Hz = find(frequencies > 0.5, 1);

    % Find the maximum amplitude value after 0.5 Hz
    [maxY_after_05Hz, maxIndex_after_05Hz] = max(amplitude_spectrum(idx_05Hz:end));
    
    % Plot a blue dot at the maximum point after 0.5 Hz
    plot(frequencies(idx_05Hz + maxIndex_after_05Hz - 1), maxY_after_05Hz, 'ro', 'MarkerSize', 7);
    text(frequencies(idx_05Hz + maxIndex_after_05Hz - 1) + 20, maxY_after_05Hz, [' Amplitude = ' num2str(maxY_after_05Hz) ' mm'], 'Color', 'red');
